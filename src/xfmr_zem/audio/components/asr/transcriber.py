...
# Setup paths for icefall (must be before imports from icefall_scripts)
_ASR_DIR = Path(__file__).parent
_ICEFALL_SCRIPTS_DIR = _ASR_DIR / "icefall_scripts"
_ICEFALL_LIB_DIR = _ASR_DIR / "icefall"

# Add paths if not already present
if str(_ICEFALL_SCRIPTS_DIR) not in sys.path:
    sys.path.append(str(_ICEFALL_SCRIPTS_DIR))
if str(_ICEFALL_LIB_DIR) not in sys.path:
    sys.path.append(str(_ICEFALL_LIB_DIR))
...
try:
    ...
    # Imports from icefall_scripts (copied from VieASR)
    from train import get_params, get_model, get_parser
    from beam_search import greedy_search_batch, modified_beam_search
    from icefall.utils import AttributeDict
    from export import num_tokens # For vocab_size
